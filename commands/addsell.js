...
module.exports = {
    ...
    .setName('addsell')
    .setDescription('Add a SELLING order to the listing')
	...
	async execute(interaction) {
        ...
        var author = interaction.member.id.toString();
              ...
              async function cmd(){
                const query = await new Promise((resolve,reject) => {
                  db.all(sql, [author], (err, rows) => {
                    ...
                    rows.forEach((row) => {
                      interaction.reply("Added **selling** " + itemcount + " " + item + " for " + price + "g by seller <@" + author + "> at /n spawn " + nation);
                      interaction.guild.channels.cache.get(logid).send("[<@" + interaction.member.id + ">]" + "  |  Added **selling** " + itemcount + " " + item + " for " + price + "g by seller <@" + author + "> at /n spawn " + nation);
              db.run(sqle,[itemcount,item,price,author,nation,rapp,"true",other,"false"]);
                    });
