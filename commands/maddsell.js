...
module.exports = {
    ...
    .setName('maddsell')
    .setDescription('[mod] Add a SELLING order to the listing')
              ...
              async function cmd(){
                const query = await new Promise((resolve,reject) => {
                  db.all(sql, [author], (err, rows) => {
                    ...
                    rows.forEach((row) => {
                      interaction.reply("Added **selling** " + itemcount + " " + item + " for " + price + "g by buyer <@" + author + "> at /n spawn " + nation);
              interaction.guild.channels.cache.get(logid).send("[<@" + interaction.member.id + ">]" + "  |  Added **selling** " + itemcount + " " + item + " for " + price + "g by buyer <@" + author + "> at /n spawn " + nation);
